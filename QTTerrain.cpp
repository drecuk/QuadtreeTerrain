...
QTTerrain::QTTerrain(char *terrainFilename, const int width, const int length, float scaleH, float scale, int _normalsFlag)
{
	...
	/*
	// allocate an array of rows Vector3f in the memory
	terrainData = new Vector3f*[width];

	// allocation new cols to the terrain[row] - the next dimension of Vector3f array
	for(int i = 0; i < width; i++)
		terrainData[i] = new Vector3f[length];	// allocate the col to the row

	// changing elements of the array
	cout<<"------------->> changing elements of the array"<<endl;
	terrainData[0][0] = Vector3f(2, 3, 4);
	cout<<"terrain[0][0] = "<<terrainData[0][0].x<<" "<<terrainData[0][0].y<<" "<<terrainData[0][0].z<<endl;
	terrainData[2][3] = Vector3f(4, 3, 2);
	cout<<"terrain[2][3] = "<<terrainData[2][3].x<<" "<<terrainData[2][3].y<<" "<<terrainData[2][3].z<<endl;
	*/
	...
	/*
	cout<<">> Initialising 2D <Vector> terrainNormals"<<endl;
	for(int x = 0; x < width; x++)
	{
		terrainNormals.push_back( vector<Vector3f>() );
		for(int z = 0; z < length; z++)
		{
			terrainNormals[x].push_back(Vector3f(0.0f, 0.0f, 0.0f));
		}
	}
	*/

	/*
	cout<<">> Initialising 2D <Vector> heightField"<<endl;
	// allocate an array of rows Vector3f in the memory
	heightField = new uint8_t*[width];

	// allocation new cols to the terrain[row] - the next dimension of Vector3f array
	for(int z = 0; z < width; z++)	heightField[z] = new uint8_t[length];	// allocate the col to the row
